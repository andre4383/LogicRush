...
static void EnsurePlayerLabTex(void) {
    if (!texPlayerLabLoaded) {
        texPlayerLab = LoadTexture("assets/images/player_spritesheet.png");
...
static void DrawPlayerLabSprite(float px, float py, float radius, float t, bool nearBarrier) {
    EnsurePlayerLabTex();
    if (!texPlayerLabLoaded) {
        // Fallback: original green circle
        float glowRadius = radius + 4.0f + sinf(t * 12.0f) * 2.0f;
        DrawCircle((int)px, (int)py, (int)glowRadius, ColorAlpha(COLOR_NEON_GREEN, 0.20f));
        DrawCircleV((Vector2){px,py}, radius, COLOR_NEON_GREEN);
        DrawCircleV((Vector2){px,py}, radius-4.0f, (Color){187,247,208,255});
        return;
    }
    float fw = floorf((float)texPlayerLab.width  / PLB_COLS);
    float fh = floorf((float)texPlayerLab.height / PLB_ROWS);
    // Frame: idle=0, alert=1, block=2, hit=3
    int frame = nearBarrier ? 2 : ((int)(t*2.0f))%2;
    int col   = frame % PLB_COLS, row = frame / PLB_COLS;
    float inset = 1.0f;
    Rectangle src = {col*fw+inset, row*fh+inset, fw-inset*2, fh-inset*2};
    float dstSz = radius * 3.8f;
    Rectangle dst = {px - dstSz*0.5f, py - dstSz*0.5f, dstSz, dstSz};
    DrawTexturePro(texPlayerLab, src, dst, (Vector2){0,0}, 0.0f, WHITE);
    // Glow ring
...
static void ResetLevel(void) {
    // (ranking trigger flag is reset by caller in UpdateGameplayScreen)
...
void UpdateGameplayScreen(void) {
    ...
    if (gameOver) {
        ...
        if (!rankingTrigLab) {
            rankingTrigLab = true;
            Ranking_ScreenEnter(globalScore, globalTimer, 2);
...
void DrawGameplayScreen(void) {
    ...
    // Draw player as sprite (falls back to circle if texture missing)
    bool nearBar = false;
    // Check if near any barrier
    for (int bi = 0; bi < activeNumBarriers && !nearBar; bi++) {
        float bX = (activeBarriers[bi].col + 0.5f) * CELL_SIZE;
        float bY = (activeBarriers[bi].row + 0.5f) * CELL_SIZE;
        float dd = sqrtf((playerPos.x-bX)*(playerPos.x-bX)+(playerPos.y-bY)*(playerPos.y-bY));
        if (dd < CELL_SIZE*1.5f) nearBar = true;
    }
    DrawPlayerLabSprite(playerPos.x, playerPos.y, playerRadius, playerTime, nearBar);
